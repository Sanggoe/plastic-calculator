...
* 테스트가 완료 되었다면, 서비스를 제공하지 않는 이상 불필요하게 서버를 올려놓을 필요가 없었다.
* 따라서 할당했던 서버를 내려야하는데, 리소스 그룹을 모두 삭제하지 않으면 나도 모르는 과금이 부과될수도!

<br/>

![image-20201014021050092](./images/image-20201014021050092.png)

* Azure portal에서 반드시 리소스 그룹을 모두 체크하여 레소스 그룹 삭제를 해주어야 한다.
* 해당 메모리에 존재하는 모든 파일들과 함께 할당된 서버와 관련한 리소스를 모두 삭제하는 과정이다.
* 생성되는데도 시간이 오래 걸린 것 처럼.. 삭제 또한 시간이 좀 걸린다.
* 이렇게 해주는 것을 습관화 하지 않으면, 나도 모르는 사이 몇달 몇년 뒤 엄청난 과금 폭탄이 되어 돌아올지도..
